...
pub async fn create_app_state_with_pool(pool: Pool) -> Result<ApiState, Box<dyn std::error::Error>> {
    ...
    println!("-> Loaded {} group variant mappings.", group_variant_cache.read().await.len());
...
pub fn create_router(app_state: ApiState) -> Router {
    Router::new()
        ...
        .route("/variants/groups", get(handlers::group_variants::get_all).post(handlers::group_variants::add))
        .route(
