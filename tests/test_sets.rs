...
async fn test_sets_endpoints() {
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    r#"{"set_code": "test-set", "name": "Test Set"}"#,
                ))
                .unwrap(),
        )
        .await
    ...
    assert_eq!(sets[10].set_code, "test-set");
    assert_eq!(sets[10].name, "Test Set");
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header(http::header::CONTENT_TYPE, "application/json")
                .body(Body::from(
                    r#"{"set_code": "test-set", "name": "Another Test Set"}"#,
                ))
                .unwrap(),
        )
        .await
    ...
    let response = app.oneshot(Request::builder().uri("/sets").body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
